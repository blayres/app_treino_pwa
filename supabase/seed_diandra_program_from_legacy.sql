-- Aplica no Supabase o treino legado da Diandra
-- (baseado no bloco comentado em src/db/index.ts).
--
-- Como usar:
-- 1) Ajuste o filtro da Diandra no CTE "diandra_profile" se necessário.
-- 2) Cole e rode inteiro no SQL Editor do Supabase.

begin;

-- 1) Localiza o perfil da Diandra.
with diandra_profile as (
  select id
  from public.profiles
  where lower(name) = lower('Diandra')
  limit 1
)
select
  case
    when exists (select 1 from diandra_profile) then 'OK: perfil Diandra encontrado'
    else 'ERRO: perfil Diandra nao encontrado em public.profiles'
  end as status;
...
-- 3) Remove treinos atuais da Diandra para reconstruir com o programa legado.
with diandra_profile as (
  select id
  from public.profiles
  where lower(name) = lower('Diandra')
...
diandra_workouts as (
  ...
  join diandra_profile p on p.id = w.user_id
)
delete from public.workout_exercises we
using diandra_workouts dw
where we.workout_id = dw.id;
...
using diandra_profile p
where w.user_id = p.id;

-- 4) Cria os 5 treinos da semana da Diandra.
with diandra_profile as (
  select id
  from public.profiles
  where lower(name) = lower('Diandra')
...
exercise_order as (
  ...
  from (
    values
      (1, 0, 'Cadeira Extensora'),
      (1, 1, 'Leg Press 45°'),
      (1, 2, 'Abdutora 45°'),
      (1, 3, 'Afundo'),
      (1, 4, 'Agachamento Sumô'),

      (2, 0, 'Graviton'),
      (2, 1, 'Remada na Máquina'),
      (2, 2, 'Fly Inverso'),
      (2, 3, 'Desenvolvimento no Banco'),
      (2, 4, 'Elevação Lateral'),

      (3, 0, 'Prancha'),
      (3, 1, 'Abdominal Crunch'),
      (3, 2, 'Elevação de Pernas'),
      (3, 3, 'Lombar'),
      ...
      (4, 2, 'Cadeira Flexora'),
      (4, 3, 'Abdutora 45°'),
      (4, 4, 'Agachamento Sumô'),

      (5, 0, 'Graviton'),
      (5, 1, 'Supino na Máquina'),
      (5, 2, 'Remada na Máquina'),
      (5, 3, 'Fly Inverso'),
...
-- where lower(p.name) = lower('Diandra')
